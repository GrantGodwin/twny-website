...
| **Current Work Item** | Item 7 — Homepage closing section ("the handshake") |
| **Repository Status** | Hero (Item 1), "What we look after" (Item 2), "Why TWNY" (Item 3), and "Helping businesses like yours" / client stories (Item 4) are all reviewed and complete. "How we work" and "The offer" were removed as standalone homepage sections during Why TWNY's development (`offerCards` data preserved in `src/data/offer.ts` for later). **Item 5 (standalone Pricing summary) has been removed by structural decision** (see below); the homepage no longer carries a pricing section. Item 6 (FAQ) is deferred. **Item 7 — the closing section — is implemented and awaiting review** (it replaces both the old Pricing section and the old conversion CTA band in `src/pages/index.astro`). |
| **Awaiting Human Direction** | Review of the implemented closing section across desktop/tablet/mobile. (A light, warm off-white prototype was tried and **rejected** — it read as leaving the site for a brochure and broke the dark page's emotional flow; the section now stays inside the locked Ink-first system on `bg-band`, using space rather than colour for the closing exhale.) Also open: the dead `/#services` anchor (header nav "Services" link, "What we look after"'s CTA, and now the closing section's "Explore our services" secondary link all point to it) and the broader Services IA decision; the Phase 3 Privacy page scope (undefined); and pricing figure sign-off (currently draft per `PricingFlag.astro`). Minor polish/follow-up items for Why TWNY and Item 4 sit in the Product Backlog. |
| **Recommended Next Action** | Review the closing section in the browser at all three breakpoints and confirm the copy/composition. Do not commit until approved. |
...
- [x] ~~5. Pricing summary~~ — **removed as a standalone homepage section** (was: a short, scannable pricing teaser driving to `/pricing`, positioned after Why TWNY). **Design rationale:** the homepage now ends by *removing the hesitation around getting in touch* rather than by discussing pricing — the last objection a warm reader holds is "I don't even know what I need," not "what does it cost." Pricing remains available elsewhere on the site (the `/pricing` page, header nav) and is **no longer a homepage feature**; the art direction's "no hidden pricing" rule is satisfied by pricing staying published and reachable, not by a homepage teaser. The `/pricing` page itself is unchanged. The section's old slot in `src/pages/index.astro` is removed.
...
- [x] **7. Homepage closing section — "the handshake"** *(was "Contact"; absorbs the old closing CTA band, and the build-order slot the removed Pricing summary used to occupy)* — **implemented, awaiting review.** The final homepage section. It is deliberately *not* another feature, pricing, or process section: a calm, spacious, reassuring close whose single job is to remove the last hesitation before getting in touch — the worry "I don't even know what I need, I'll waste their time." It resolves that in the brand's own voice ("Working out what you need is our job, not yours"), which is the homepage's central "the jargon stops with us" promise applied to the first conversation. Eyebrow "Getting started"; headline "Working out what you need is our job."; body speaking directly to the visitor — you don't need to know the technology, have the answer, or arrive with a detailed brief; tell us what's happening in your own words and we'll help work out the right place to start (no invented customer quotes); primary CTA "Tell us what you need" → `/contact`; secondary, visually quiet link "Explore our services" → `/#services` (the same anchor the header nav and "What we look after" already use; resolves with the Services IA decision). Replaces **both** the standalone Pricing section and the old conversion CTA band in `src/pages/index.astro`. **Surface:** stays inside the locked Ink-first dark system on `bg-band`, the lightest of the dark surfaces — the page lifts gently out of the darkest section (Client Stories on `bg-sand`) and settles into the footer (`bg-paper`). A light/off-white prototype was tried and rejected for breaking the dark page's emotional flow; the closing exhale is now made with **space, not colour** (the most generously padded section on the page). Left-aligned, single text column; no cards, icons, diagrams, illustrations, or process steps.
