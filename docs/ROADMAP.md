...
| Field | Value |
| --- | --- |
| **Current Phase** | Phase 4 — Launch (custom-domain closeout) |
| **Current Sprint** | Custom-domain production closeout — merged and live on `https://www.twny.com.au`. |
| **Current Work Item** | None in flight. The site is launched on its real domain with clean search-engine signals. |
...
| **Repository Status** | Launched. The closeout pass corrects the canonical origin to `https://www.twny.com.au` (it was still emitting `twny-website.vercel.app` in production), adds permanent redirects for the legacy WordPress routes, adds a branded 404, replaces Astro's default rocket favicon with the wordmark icon set, tightens the reveal motion, and adds `npm run verify` — a build-output check that fails the build on exactly the class of regression that caused this pass. **NO PRICING remains locked.** |
...
- [x] **Motion** — the hero heading previously sat at zero opacity for 500 ms and settled at ~1.2 s, and the wordmark ran to 3 s. Now: heading starts revealing at ~124 ms, is half visible at ~268 ms and settled at ~518 ms; wordmark settles at ~918 ms. Scroll reveals shortened from 700 ms to 450 ms and the observer pre-triggers 12% of a viewport below the fold, so a fast scroll no longer lands on a section that is still blank. Direction, easing and subtlety unchanged; reduced motion still shows everything immediately.
- [x] **Build-output verification** — `npm run verify` (build, then `scripts/check-build.mjs`). 72 assertions covering the canonical host and per-page canonical path, `og:`/`twitter:` URLs, title/description presence and uniqueness, one `<h1>` per page, absence of any `vercel.app`/`localhost` string in the output, sitemap and robots contents, no public pricing in rendered text, no retired scaffold proof or legacy positioning, a single consistent `mailto:`, icon presence and references, internal links resolving to built pages, and the 404's status markers. Also `npm run check` (`astro check`): 0 errors, 0 warnings.
