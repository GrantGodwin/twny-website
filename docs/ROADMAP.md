...
| **Current Work Item** | Item 4 — "Helping businesses like yours" / client stories (evolved from "Social proof") — **rebuilt as an image-led living feature, awaiting review** |
| **Repository Status** | Hero (Item 1), "What we look after" (Item 2), and "Why TWNY" (Item 3) are all reviewed and complete. Item 4 has been **rebuilt and is awaiting review** (see below) — it superseded the earlier static snapshot/list version with an image-led client-stories feature. "How we work" and "The offer" were removed as standalone homepage sections during Why TWNY's development (`offerCards` data preserved in `src/data/offer.ts` for later). Items 5–7 of Phase 2 are not started. |
| **Awaiting Human Direction** | **Review of Item 4** ("Helping businesses like yours" client-stories feature) before it is marked complete. Note its content is **scaffold**: five placeholder stories with reused local images standing in for real client photography (replaceable via `image`/`imageAlt` fields in `src/data/offer.ts`). Also open: the dead `/#services` anchor (header nav "Services" link and "What we look after"'s CTA both point to it) and the broader Services IA decision, the Phase 3 Privacy page scope (undefined), and pricing figure sign-off (currently draft per `PricingFlag.astro`). A handful of minor Why TWNY polish items were moved to the Product Backlog — see below. |
| **Recommended Next Action** | Review the rebuilt Item 4 client-stories feature on the homepage; once approved (and real photography/copy substituted), mark it complete and move to Item 5 (Pricing summary). |
...
- [ ] **4. Helping businesses like yours / client stories** *(evolved from "Social proof")* — **rebuilt, awaiting review.** Went through two builds. First pass was a static, typographic **business snapshots** spread (recognition-first, no photography); this was then superseded — keeping the intent, changing the execution — by an image-led **living editorial feature** for stronger lead-gen conversion and emotional proof. Current implementation (`src/components/home/ClientStories.astro`, data in `stories` in `src/data/offer.ts`): one **featured client story** at a time (large image with editorial overlay on `lg`, stacked image + panel below `lg`) carrying business type, client name, a genuine quote as the emotional anchor, attribution, a mono "what we look after" rail, and an outcome line — alongside a **persistent, always-visible roster** of all five businesses that *is* the navigation. The feature auto-advances ~5s when idle, pauses on hover/focus/interaction and when offscreen, and any roster item (keyboard-accessible `<button>`, `aria-pressed`) drives it on hover/focus/click via a filmic cross-dissolve. No arrows, dots, 01/05 counters, progress bars, slider chrome, logo walls, review stars, or cards. Respects `prefers-reduced-motion` (no auto-rotate; instant swaps) and renders the first story + full roster server-side so it stays meaningful with JS disabled. **Content is scaffold:** five placeholder stories (Deller Constructions, First Finance, Sinclair Brook, Baza Property Group, Speech Made Simple) with reused local `/images/*.webp` files standing in for real client photography — every story has `image`/`imageAlt` fields built to swap for final approved assets/copy without touching the component; nothing fabricated beyond the supplied scaffold. Positioned after "Why TWNY" and before Pricing on `bg-sand` (`#businesses-like-yours`).
